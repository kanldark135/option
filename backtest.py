...
df_main = df_kospi.assign(atm = find_closest_strike(df_kospi.close), booleanindex = df_kospi.index.isin(expiry_table_7.index))

df_main = pd.concat([df_main, expiry_table_7], axis = 1)
df_main['expiry'].fillna(method = 'ffill', inplace = True)
df_main['expiry'].mask(df_main.index > df_main['expiry'], inplace = True)
